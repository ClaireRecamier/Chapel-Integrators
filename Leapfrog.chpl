...
proc advance(dt: real){
  //define array of acceleration tuples;
  var accelerations: [0..numBodies-1] 3*real;
  //initialize relative position vector and its length as new variables
  force(accelerations);

  for i in 0..numBodies-1 do{
    bodies[i].velocity += (dt) * accelerations[i];
    bodies[i].position += dt * bodies[i].velocity;
  }
}

proc vel_halfstep(sign) {
  var accelerations: [0..numBodies-1] 3*real;
  var signed_dt: real = sign * timestep;
  //writeln("signed_dt: ",signed_dt);
  force(accelerations);
  for i in 0..numBodies-1 do {
    bodies[i].velocity += 0.5 * signed_dt * accelerations[i];
    //writeln("halfstep velocity ",0.5 * signed_dt * accelerations[i]);
  }
}
...
proc main() {
  ...
  initSun();
  writef("%.9r\n",energy());
  vel_halfstep(-1.0); //velocity halfstep backwards

  for i in 1..iterations do {
    advance(timestep);
    if i % 10000 == 0 { //print energy every 100th iteration
      vel_halfstep(1.0); //velocity halfstep forwards
      writef("%.9r\n",energy());
      vel_halfstep(-1.0); //velocity halfstep backwards
    }
  }
