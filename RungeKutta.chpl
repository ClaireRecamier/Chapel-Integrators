...
config const iterations = 1000000, timestep = 0.1;
...
proc advance(dt: real){
  ...
  //define arrays of tuples;
  var K1a: [0..numBodies-1] 3*real;
  var K2a: [0..numBodies-1] 3*real;
  var K3a: [0..numBodies-1] 3*real;
  var K4a: [0..numBodies-1] 3*real;
  var K2v: [0..numBodies-1] 3*real;
  var K3v: [0..numBodies-1] 3*real;
  var K4v: [0..numBodies-1] 3*real;
  var pos1: [0..numBodies-1] 3*real;
  var pos2: [0..numBodies-1] 3*real;
  var pos3: [0..numBodies-1] 3*real;
  var pos4: [0..numBodies-1] 3*real;

  //initialize halfstep variable
  var halfstep: real = timestep/2;

  for i in 0..numBodies-1 do { //set up pos1 array
    pos1[i] = bodies[i].position;
  }

  force(K1a,pos1); //computer forces

  //define K1a, pos2, and K2v
  for i in 0..numBodies-1 do{
    pos2[i] = bodies[i].position + (bodies[i].velocity * halfstep);
    K2v[i] = bodies[i].velocity + (K1a[i] * halfstep);
  }

  force(K2a,pos2);
  //define K2a, pos3, and K3v
  for i in 0..numBodies-1 do {
    //pos3 position calcualted at half timestep using velocity K2v
    pos3[i] = bodies[i].position + (K2v[i] * halfstep);
    K3v[i] = bodies[i].velocity + (K2a[i] * halfstep);
  }

  force(K3a,pos3);
  //define K3a, pos4, and K4v
  for i in 0..numBodies-1 do {
    pos4[i] = bodies[i].position + (K3v[i] * timestep);
    K4v[i] = bodies[i].velocity + (K3a[i] * timestep);
  }

  force(K4a,pos4);
  //define K4a
  for i in 0..numBodies-1 do {
    bodies[i].position += (bodies[i].velocity + (K2v[i] * 2) + (K3v[i] * 2) + K4v[i]) * (timestep/6.0);
    bodies[i].velocity += (K1a[i] + (K2a[i] * 2) + (K3a[i] * 2) + K4a[i]) * (timestep/6.0) ;
  }
